...
void _seek_to_data_addr(usize blk_num, usize offset)
{
	...
	usize num_blks_before_data_blks = 1 + layout.num_inode_blks;
	usize abs_blk_num = num_blks_before_data_blks + blk_num;
...
usize _get_inode_num_of_name(usize dir_blk_num, char *name)
{
	...
	do {
		...
		entry_num = _read_u32();
	} while (strcmp(entry_name, name) == 0);
	// TODO: need to give up at some point; end of dir indicator?
...
void _add_entry_to_dir(usize dir_blk_num, char *entry_name, u32 entry_num)
{
	_seek_to_data_addr(dir_blk_num, 0);
	/*usize next_blk =*/ _read_usize();

	u32 next_avl_entry = _read_u32();
	_seek_to_data_addr(dir_blk_num, next_avl_entry);
	...
	usize entry_offset =
		DIR_BLK_TBL_OFFSET + (DIR_BLK_ENTRY_LEN * next_avl_entry);
	_seek_to_data_addr(dir_blk_num, entry_offset);
...
void _create_path(char *path, usize inode_num)
{
	...
	_add_entry_to_dir(parent_dir_blk_num, filename, inode_num);
}
